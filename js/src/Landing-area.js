...
LandingArea.prototype._hitTop = function (ship){
  ...
  if (ship.y > land && (ship.x-ship.radius)>this.x && (ship.x+ship.radius)<this.x+this.width && ship.vy <= 3 && ship.angle > (75*(Math.PI/180) && ship.angle < (105*(Math.PI/180) ) )){
    ...
    clearInterval();
  } else if (ship.y > land && (ship.x-ship.radius)>this.x && (ship.x+ship.radius)<this.x+this.width && ship.vy > 3 ) {
    ship.y = land; // DESTROZO EL VALOR DE Y
    clearInterval(intervalId); // IMPIDO LA ACTUALIZACIÓN DE JUEGO
    alert("Game Over");
  };
